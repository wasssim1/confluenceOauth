...
import static com.confluence.test.oauth.PropertiesClient.ACCESS_TOKEN;
import static com.confluence.test.oauth.PropertiesClient.CONSUMER_KEY;
import static com.confluence.test.oauth.PropertiesClient.PRIVATE_KEY;
import static com.confluence.test.oauth.PropertiesClient.REQUEST_TOKEN;
import static com.confluence.test.oauth.PropertiesClient.SECRET;
...
public class OAuthClient {
    ...
    private final ConfluenceOAuthClient confluenceOAuthClient;

    public OAuthClient(PropertiesClient propertiesClient, ConfluenceOAuthClient confluenceOAuthClient) {
        this.propertiesClient = propertiesClient;
        this.confluenceOAuthClient = confluenceOAuthClient;
    ...
    private Optional<Exception> handleGetRequestTokenAction(List<String> arguments) {
        ...
        try {
        	// FIXME: Unable to retrieve the request token from Confluence
            String requestToken = confluenceOAuthClient.getAndAuthorizeTemporaryToken(properties.get(CONSUMER_KEY), properties.get(PRIVATE_KEY));
    ...
    private Optional<Exception> handleGetAccessToken(List<String> arguments) {
        ...
        try {
            String accessToken = confluenceOAuthClient.getAccessToken(tmpToken, secret, properties.get(CONSUMER_KEY), properties.get(PRIVATE_KEY));
    ...
    private Optional<Exception> handleGetRequest(List<String> arguments) {
        ...
        try {
            OAuthParameters parameters = confluenceOAuthClient.getParameters(tmpToken, secret, properties.get(CONSUMER_KEY), properties.get(PRIVATE_KEY));
